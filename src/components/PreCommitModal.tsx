...
import { Lock, ShieldCheck, ArrowRight, Wallet, X, Code2, Loader2 } from 'lucide-react';
      ...
      <div className="glass-panel-glow max-w-lg w-full p-6 space-y-5 animate-in fade-in zoom-in duration-200">
        ...
        <div className="bg-slate-900/80 p-4 rounded-xl border border-slate-800 space-y-3 font-mono text-xs">
          <div className="flex justify-between items-center text-slate-300">
            ...
            <span className="text-indigo-400 font-bold">{walletState.address.slice(0, 8)}...{walletState.address.slice(-4)}</span>
          </div>
          ...
          <div className="flex justify-between items-center text-slate-400">
            ...
            <span className="text-white">Ethereum ({walletState.networkName})</span>
          </div>
          ...
          <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-500 font-mono">ChainId: {typedData.domain.chainId}</span>
          </div>
          ...
          {showEip712Details && (
            <div className="mt-2 p-3 rounded-lg bg-slate-950 border border-indigo-900/80 text-[10px] space-y-2 overflow-x-auto text-indigo-200">
              ...
              <div>
                <span className="font-bold text-white block mb-0.5">Primary Type & Struct:</span>
                <pre>{JSON.stringify(typedData.types, null, 2)}</pre>
              </div>
            </div>
          )}
