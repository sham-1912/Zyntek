...
export const NetworkHealthAndActivityLog: React.FC<NetworkHealthAndActivityLogProps> = ({ logs }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="bg-[#0E1E38] border border-[#8DC2FF]/20 rounded-2xl p-5 space-y-3.5 shadow-xl">
        <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
          ...
          <span className="text-[10px] font-mono text-[#CEF26D] bg-[#142848] px-2 py-0.5 rounded font-bold border border-[#CEF26D]/30">
            Health 98.4%
          </span>
        </div>
      ...
      <div className="bg-[#0E1E38] border border-[#8DC2FF]/20 rounded-2xl p-5 space-y-3 shadow-xl">
        <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
          <div className="flex items-center gap-2">
        ...
        <div className="bg-[#070F1E] p-3 rounded-xl border border-white/5 h-44 overflow-y-auto space-y-1.5 font-mono text-[11px]">
          {logs.length === 0 ? (
            <div className="text-[#CBD5E1]/60 py-8 text-center">
              Awaiting intent broadcast to stream live execution events...
            </div>
          ) : (
            logs.map((log) => (
              <div key={log.id} className="leading-tight flex items-start gap-1.5">
                <span className="text-[#8DC2FF]/80 text-[10px] shrink-0">[{log.timestamp}]</span>
                <span
                  className={
                    log.type === 'success'
                      ? 'text-[#CEF26D]'
